...
class SystemStateProvider with ChangeNotifier {
  ...
  Map<String, Map<String, double>> get vacuumSystemStatus =>
      _vacuumSystemStatus;
  ...
  void updateVacuumSystemStatus(
      String component, String parameter, double value) {
  ...
  void simulateVacuumSystem() {
    ...
    double roughingPumpPressure =
        _vacuumSystemStatus['Roughing Pump']!['pressure']!;
    ...
    if (roughingPumpPressure > 1e-3) {
      ...
      updateVacuumSystemStatus(
          'Roughing Pump', 'pressure', roughingPumpPressure);
    }
  ...
  void updatePrecursorLevel(String precursor, double level) {
    if (_precursorLevels.containsKey(precursor)) {
      ...
      if (level < 20.0) {
        addNotification(
            '$precursor level is low (${level.toStringAsFixed(1)}%)',
  ...
  void updateDashboardWidgetOrder(List<String> newOrder) {
    ...
    _systemStateRepository.saveDashboardWidgetOrder(
        _authService.currentUserId!, newOrder);
    notifyListeners();
  }
  ...
  Future<void> loadDashboardWidgetOrder() async {
    ...
    if (userId != null) {
      _dashboardWidgetOrder =
          await _systemStateRepository.getDashboardWidgetOrder(userId);
      notifyListeners();
    }
  }
  ...
  void runDiagnostic(String componentName) async {
    addNotification(
        'Running diagnostic for $componentName', NotificationSeverity.info);
    ...
    if (diagnosticPassed) {
      addNotification('Diagnostic for $componentName completed successfully',
          NotificationSeverity.info);
    } else {
      addNotification(
          'Diagnostic for $componentName failed', NotificationSeverity.warning);
    }
  }
  ...
  void addNotification(String message, NotificationSeverity severity) {
    ...
    _systemStateRepository.saveNotification(
        _authService.currentUserId!, notification);
    notifyListeners();
  }
  ...
  void setComponentSetValue(
    ...
    if (userId != null) {
      if (_components.containsKey(componentName)) {
        ...
        _systemStateRepository.saveComponent(
            userId, _components[componentName]!);
