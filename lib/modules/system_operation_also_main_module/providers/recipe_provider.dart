...
class RecipeProvider with ChangeNotifier {
  ...
  Recipe? getPreviousVersion(String id, int currentVersion) {
    ...
    int index =
        versions.indexWhere((recipe) => recipe.version == currentVersion);
  ...
  Map<String, dynamic> compareRecipeVersions(
      Recipe oldVersion, Recipe newVersion) {
    ...
    if (oldVersion.chamberTemperatureSetPoint !=
        newVersion.chamberTemperatureSetPoint) {
    ...
    if (oldVersion.pressureSetPoint != newVersion.pressureSetPoint) {
      differences['pressureSetPoint'] = {
        'old': oldVersion.pressureSetPoint,
        'new': newVersion.pressureSetPoint
      };
    }
  ...
  List<Map<String, dynamic>> _compareSteps(
      List<RecipeStep> oldSteps, List<RecipeStep> newSteps) {
    List<Map<String, dynamic>> stepDifferences = [];

    int maxLength =
        oldSteps.length > newSteps.length ? oldSteps.length : newSteps.length;

    for (int i = 0; i < maxLength; i++) {
      if (i < oldSteps.length && i < newSteps.length) {
        if (oldSteps[i].type != newSteps[i].type ||
            !_areParametersEqual(
                oldSteps[i].parameters, newSteps[i].parameters)) {
          stepDifferences.add({
            'index': i,
        ...
        if (oldSteps[i].type == StepType.loop &&
            newSteps[i].type == StepType.loop) {
          var subStepDifferences = _compareSteps(
              oldSteps[i].subSteps ?? [], newSteps[i].subSteps ?? []);
    ...
    switch (step.type) {
      case StepType.loop:
        return 'Loop ${step.parameters['iterations']} times' +
            (step.parameters['temperature'] != null
                ? ' (T: ${step.parameters['temperature']}°C)'
                : '') +
            (step.parameters['pressure'] != null
                ? ' (P: ${step.parameters['pressure']} atm)'
                : '');
      case StepType.valve:
        return '${step.parameters['valveType'] == ValveType.valveA ? 'Valve A' : 'Valve B'} for ${step.parameters['duration']}s' +
            (step.parameters['gasFlow'] != null
                ? ' (Flow: ${step.parameters['gasFlow']} sccm)'
                : '');
      case StepType.purge:
